...
import {
  ...
  securityRequestInspectorMiddleware,
  createEndpointRateLimiter
} from './server/securityManager';
import { generateToken, authenticateToken, optionalAuth, requireAdmin, requireSuperAdmin, AuthRequest } from './server/auth';

async function startServer() {
  const catalogSync = await db.syncFromSupabase();
  if (catalogSync.success) {
    console.log(`✅ Supabase catalog ready (${catalogSync.anime} anime, ${catalogSync.seasons} seasons, ${catalogSync.episodes} episodes)`);
  }
  ...
  // Security Headers & Threat Inspector Middlewares
  app.use(securityHeadersMiddleware);
  app.use(securityRequestInspectorMiddleware);
  ...
  // User / Admin Login
  app.post('/api/auth/login', createEndpointRateLimiter('auth_login', 15), (req, res) => {
    try {
      const cooldownCheck = securityManager.isLoginInCooldown(req);
      if (cooldownCheck.inCooldown) {
        res.status(429).json({
          error: 'Too many failed login attempts. Security protection active. Please try again later.',
          retryAfterSeconds: cooldownCheck.remainingSeconds
        });
        return;
      }
      ...
      if (!user) {
        securityManager.recordLoginFailure(req, cleanIdentifier);
      ...
      securityManager.recordLoginSuccess(req);
  ...
  app.post('/api/auth/admin-login', createEndpointRateLimiter('auth_admin', 10), (req, res) => {
    try {
      const cooldownCheck = securityManager.isLoginInCooldown(req);
      if (cooldownCheck.inCooldown) {
        res.status(429).json({
          error: 'Too many failed login attempts. Administrator account protection active.',
          retryAfterSeconds: cooldownCheck.remainingSeconds
        });
        return;
      }
  ...
  app.post('/api/anime/:id/comments', createEndpointRateLimiter('comments', 15), (req, res) => {
    const { id } = req.params;
    const { username, content } = req.body;
    const validation = securityManager.validateCommentInput(username, content);
    if (!validation.isValid) {
      res.status(400).json({ error: validation.error || 'Invalid comment input.' });
      return;
    }
    const anime = db.getAnimeById(id);
    if (!anime) {
    ...
    const comment = db.addComment(id, validation.cleanUsername!, validation.cleanContent!);
    res.json({ comment });
  });
  ...
  app.get('/api/search', createEndpointRateLimiter('search', 30), (req, res) => {
  ...
  // ==========================================
  // SECURITY & THREAT PROTECTION API
  // ==========================================

  app.get('/api/admin/security/summary', requireAdmin, (req, res) => {
    try {
      res.json(securityManager.getSecuritySummary());
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Error fetching security summary.' });
    }
  });

  app.get('/api/admin/security/incidents', requireAdmin, (req, res) => {
    try {
      const status = req.query.status as string;
      res.json({ incidents: securityManager.getIncidents(status) });
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Error fetching security incidents.' });
    }
  });

  app.post('/api/admin/security/incidents/:id/status', requireAdmin, (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      const { status } = req.body;
      const success = securityManager.updateIncidentStatus(id, status);
      if (!success) {
        res.status(404).json({ error: 'Incident not found.' });
        return;
      }
      db.logAdminAction(req.user!.id, req.user!.username, 'Security Incident Updated', `Set incident ${id} status to ${status}`, req.ip);
      res.json({ message: `Incident status updated to ${status}`, success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Error updating incident status.' });
    }
  });

  app.get('/api/admin/security/blocked-sources', requireAdmin, (req, res) => {
    try {
      res.json({ blockedSources: securityManager.getBlockedSources() });
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Error fetching blocked sources.' });
    }
  });

  app.post('/api/admin/security/blocked-sources/:id/clear', requireAdmin, (req: AuthRequest, res) => {
    try {
      const { id } = req.params;
      const success = securityManager.clearBlockedSource(id);
      if (!success) {
        res.status(404).json({ error: 'Blocked source record not found.' });
        return;
      }
      db.logAdminAction(req.user!.id, req.user!.username, 'Security Block Cleared', `Manually unblocked source ${id}`, req.ip);
      res.json({ message: 'Temporary block manually cleared.', success: true });
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Error clearing blocked source.' });
    }
  });

  app.get('/api/admin/security/logs', requireAdmin, (req, res) => {
    try {
      const { search, severity, eventType } = req.query;
      ...
      res.status(500).json({ error: err.message || 'Error fetching security logs.' });
    }
  });

  app.get('/api/admin/security/settings', requireAdmin, (req, res) => {
    try {
      res.json({ settings: securityManager.getSettings() });
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Error fetching security settings.' });
    }
  });

  app.put('/api/admin/security/settings', requireAdmin, (req: AuthRequest, res) => {
    try {
      const updated = securityManager.updateSettings(req.body);
      db.logAdminAction(req.user!.id, req.user!.username, 'Security Settings Updated', 'Modified platform security settings', req.ip);
      res.json({ message: 'Security settings saved successfully.', settings: updated });
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Error updating security settings.' });
    }
  });
  ...
  // Delete Season
  app.delete('/api/admin/anime/:animeId/seasons/:seasonNumber', requireAdmin, (req: AuthRequest, res) => {
    try {
      ...
      const anime = db.getAnimeById(animeId);
      if (!anime) {
        res.status(404).json({ error: 'Anime/Content record not found.' });
        return;
      }

      const result = db.deleteSeason(anime.id, sNum);
      db.logAdminAction(
        req.user!.id,
        req.user!.username,
        'Deleted Season',
        `Deleted Season ${sNum} (${result.deletedCount} episodes) from "${anime.title}" (ID: ${anime.id})`,
        req.ip
      );

      res.json({ message: `Season ${sNum} deleted successfully (${result.deletedCount} episodes removed).`, deletedCount: result.deletedCount });
    } catch (err: any) {
      res.status(500).json({ error: err.message || 'Failed to delete season.' });
    }
  });
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    const isR2Configured = Boolean(process.env.R2_ACCOUNT_ID && process.env.R2_ACCESS_KEY_ID && process.env.R2_BUCKET_NAME);

    console.log(`=========================================`);
    console.log(`🎬 Cinevora Server running on port ${PORT}`);
    console.log(`📦 Primary Storage Driver: ${storageDriver.toUpperCase()}`);
    console.log(`☁️  Cloudflare R2 Media:    ${isR2Configured ? 'CONFIGURED' : 'LOCAL FALLBACK (/public/uploads/)'}`);
    console.log(`🛡️  Fallback Mode:          ${process.env.ENABLE_LOCAL_JSON_FALLBACK !== 'false' ? 'ENABLED' : 'DISABLED'}`);
    console.log(`=========================================`);
  });
}
