...
class Database {
  ...
  /**
   * Loads the catalog from Supabase when configured. The rest of Cinevora's
   * application state (users, comments, settings, ads, etc.) remains backed
   * by the existing JSON database so this migration is intentionally low-risk.
   */
  public async syncFromSupabase(): Promise<{ success: boolean; anime: number; seasons: number; episodes: number; error?: string }> {
    if (!isSupabaseConfigured()) {
      return { success: false, anime: 0, seasons: 0, episodes: 0, error: 'Supabase is not configured.' };
    }

    try {
      const catalog = await fetchSupabaseCatalog();
      const seasonsById = new Map(catalog.seasons.map(s => [s.id, s]));

      this.data.anime = catalog.anime.map((row: any) => ({
        id: String(row.id),
        title: row.title || '',
        slug: String(row.title || row.id).toLowerCase().trim().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, ''),
        description: row.description || '',
        poster: row.poster_url || '',
        backdrop: row.backdrop_url || row.poster_url || '',
        banner: row.backdrop_url || row.poster_url || '',
        thumbnail: row.poster_url || '',
        genres: Array.isArray(row.genres) ? row.genres : [],
        year: Number(row.year || 0),
        rating: Number(row.rating || 0),
        type: (['ANIME', 'MOVIE', 'SERIES'].includes(String(row.type || '').toUpperCase()) ? String(row.type).toUpperCase() : 'ANIME') as Anime['type'],
        status: String(row.status || '').toUpperCase() === 'ONGOING' ? 'ONGOING' : 'COMPLETED',
      ...
      this.data.episodes = catalog.episodes
        .map((row: any) => {
          const season = seasonsById.get(String(row.season_id));
          if (!season) return null;
          return {
            id: String(row.id),
            anime_id: String(season.anime_id),
      ...
      this.saveData();
      console.log(`☁️ Supabase catalog loaded: ${this.data.anime.length} anime, ${catalog.seasons.length} seasons, ${this.data.episodes.length} episodes`);
      return { success: true, anime: this.data.anime.length, seasons: catalog.seasons.length, episodes: this.data.episodes.length };
    } catch (err: any) {
      const message = err?.message || String(err);
      console.warn(`⚠️ Supabase catalog sync failed; keeping local JSON data. ${message}`);
      return { success: false, anime: 0, seasons: 0, episodes: 0, error: message };
    }
  }
