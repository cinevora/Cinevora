...
export const API = {
  ...
  async deleteSeason(animeId, seasonNumber) {
    const res = await fetch(`${API_BASE}/admin/anime/${animeId}/seasons/${seasonNumber}`, {
      method: 'DELETE',
      headers: getHeaders(),
      credentials: 'include'
    });
  ...
  async getSecuritySummary() {
    const res = await fetch(`${API_BASE}/admin/security/summary`, { headers: getHeaders(), credentials: 'include' });
    return parseJsonResponse(res);
  },

  async getSecurityIncidents(status = '') {
    const params = new URLSearchParams();
    if (status) params.append('status', status);
    const res = await fetch(`${API_BASE}/admin/security/incidents?${params.toString()}`, { headers: getHeaders(), credentials: 'include' });
    return parseJsonResponse(res);
  },

  async updateSecurityIncidentStatus(id, status) {
    const res = await fetch(`${API_BASE}/admin/security/incidents/${id}/status`, {
      method: 'POST',
      headers: getHeaders({ 'Content-Type': 'application/json' }),
      credentials: 'include',
      body: JSON.stringify({ status })
  ...
  async getBlockedSources() {
    const res = await fetch(`${API_BASE}/admin/security/blocked-sources`, { headers: getHeaders(), credentials: 'include' });
    return parseJsonResponse(res);
  },

  async clearBlockedSource(id) {
    const res = await fetch(`${API_BASE}/admin/security/blocked-sources/${id}/clear`, {
      method: 'POST',
      headers: getHeaders(),
      credentials: 'include'
    });
  ...
  async getSecurityLogs(search = '', severity = '', eventType = '') {
    const params = new URLSearchParams();
    if (search) params.append('search', search);
    if (severity) params.append('severity', severity);
    if (eventType) params.append('eventType', eventType);
    const res = await fetch(`${API_BASE}/admin/security/logs?${params.toString()}`, { headers: getHeaders(), credentials: 'include' });
    return parseJsonResponse(res);
  },

  async getSecuritySettings() {
    const res = await fetch(`${API_BASE}/admin/security/settings`, { headers: getHeaders(), credentials: 'include' });
    return parseJsonResponse(res);
  },
